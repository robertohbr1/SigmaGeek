...
const ArquivoEntrada = 'C:\Temp\Lab\Fase1\input.txt';
...
begin
  ...
  begin
    ...
    xv := vKeys[x1][nLenKey - 2];
    yv := vKeys[x1][nLenKey - 1];
...
begin
  ...
  begin
    ...
    xv := vCopy[x1][nLenCopy - 2];
    yv := vCopy[x1][nLenCopy - 1];
...
begin
  ...
  for x := 0 to High(vKeys[0]) - 2 do
  begin
    s := s + vDir[vKeys[0][x]] + ' ';
  end;
  ts.Add(s);
  ts.Add(SecondsBetween(Start, Now).ToString);
  ts.Add(IntToStr(Length(vKeys[0]) - 2));
